...
function App(){
  ...
  const hasBuildingValue=Boolean(state&&(state.meter_online||active));
  ...
  return <div className="app-shell">
    <aside className="sidebar"><a className="brand" href="#" onClick={e=>{e.preventDefault();setPage('overview');}}><span className="brand-symbol"><Zap size={23} fill="currentColor"/></span><span>ladepark<span className="brand-sub">ENERGIE IM GLEICHGEWICHT</span></span></a>
      ...
      <div className="sidebar-bottom"><div className="local-info"><span className={`status-dot ${offline?'red':''}`}/><div><strong>{offline?'Instanz nicht erreichbar':'Lokale Instanz'}</strong><small>Python · FastAPI · SQLite</small></div></div><span className="version">MVP 0.2 <span>{active?'AKTIV':commissioning?'NUR LESEN':'SIMULATION'}</span></span></div>
    </aside>
    <main><header className="topbar"><div className="breadcrumb">Ladepark <ChevronRight size={13}/><strong>{nav.find(n=>n.id===page)?.label}</strong></div><div className="top-right"><span className={`simulation-pill ${active?'active':''}`}>{hardware?<PlugZap size={13}/>:<FlaskConical size={13}/>} {active?'Regelung aktiv':commissioning?'Nur-Lese-Modus':'Simulationsmodus'}</span><span className="avatar">PK</span></div></header>
    <div className="content"><div className="page-heading"><div><p className="eyebrow">ENERGIEMANAGEMENT</p><h1>{page==='overview'?(active?'Lokale Regelung aktiv.':commissioning?'Echte Geräte im lokalen Netz.':'Alles im Gleichgewicht.'):page==='simulation'?'Szenarien ausprobieren.':page==='commissioning'?'Ladestationen verwalten.':page==='settings'?'Dein Ladepark. Deine Regeln.':'Jede Änderung im Blick.'}</h1><p className="subtitle">{page==='overview'?(active?'Die Ladefreigaben werden anhand des konfigurierten Fallback-Verbrauchs verteilt.':commissioning?'Live-Verbindung zu den KEBA-Ladestationen im Nur-Lese-Modus.':'Ein Anschluss. Vier Ladepunkte. Intelligent verteilt.'):page==='simulation'?'Teste den Regler mit wechselnder Last und unterbrochenen Verbindungen.':page==='commissioning'?'IP-Adressen speichern, Modbus prüfen und die KEBA-Weboberfläche öffnen.':page==='settings'?'Die lokale Instanz prüft und bestätigt jede Änderung.':'Regelzustände und bestätigte Einstellungen aus der lokalen Instanz.'}</p></div>
    ...
    {!state?<div className="loading"><LoaderCircle className="spin"/><h2>{offline?'Auf lokale Instanz warten':'Ladepark wird verbunden'}</h2><p>Messwerte werden von der lokalen FastAPI abgerufen.</p></div>:<>
      ...
      {active&&!offline&&<div className="alert active-control" role="status"><ShieldCheck size={21}/><div><strong>Aktive Regelung mit Fallback</strong><p>Da kein Gebäudenzähler verbunden ist, reserviert der Regler dauerhaft {number(state.settings.fallback_building_kw)} kW als Gebäudeverbrauch.</p></div><button className="text-button" onClick={()=>setPage('settings')}>Fallback anpassen <ArrowRight size={15}/></button></div>}
      {state.status==='safe'&&!offline&&<div className="alert warning" role="alert"><ShieldCheck size={21}/><div><strong>Sicherer Halt aktiv</strong><p>Alle Ladepunkte sind pausiert.</p></div></div>}
      ...
      {page==='overview'&&<>
        ...
        <section className="overview-grid"><article className="panel chart-panel"><div className="panel-heading"><div><h2>Leistung im Verlauf</h2><p>{hardware?'Messwerte der über Modbus verbundenen Ladepunkte':'Echte Messpunkte der laufenden Simulation'}</p></div><span className="quiet-tag"><Clock3 size={12}/> Letzte 15 Minuten</span></div><div className="chart-legend"><span><i className="legend green"/> {hardware?'Ladeleistung':'Netzbezug'}</span>{!commissioning&&<><span><i className="legend grey"/> {active?'Fallback':'Gebäude'}</span><span><i className="limit-line"/> Anschlussgrenze</span></>}</div><Chart samples={samples} limit={state.settings.power_limit_kw}/></article>
          <article className="panel phase-panel"><div className="panel-heading"><div><h2>Phasen im Blick</h2><p>Strom je Außenleiter</p></div><Activity size={18}/></div>{state.phase_currents_a.map((a,i)=><div className="phase" key={i}><div><strong>L{i+1}</strong><span>{hardware?(state.stations.some(s=>s.online)?number(a):'—'):(state.meter_online?number(a):'—')} <small>/ {state.settings.phase_limit_a} A</small></span></div><div className="phase-track"><i className={a>state.settings.phase_limit_a?'over':''} style={{width:`${Math.min(100,a/state.settings.phase_limit_a*100)}%`}}/></div></div>)}<div className="phase-note"><CircleHelp size={13}/><span>{active?'Fallback-Last wird gleichmäßig auf drei Phasen angesetzt.':commissioning?'Ohne Gebäudemessung werden nur Ladepunktströme angezeigt.':`${state.settings.phase_limit_a} A ist eine Simulationsannahme.`}</span></div></article></section>
      ...
      {page==='commissioning'&&hardware&&<div className="settings-grid commissioning-grid">{state.stations.map(s=><CommissioningStation key={s.id} station={s} disabled={disabled} onSave={patch=>command('station',patch,s.id)}/>)}</div>}
      {page==='settings'&&settings&&<div className="settings-grid"><section className="panel form-panel"><div className="panel-heading"><div><h2>Standort & Grenzen</h2><p>{active?'Diese Werte steuern die realen Ladefreigaben.':commissioning?'Die Grenzwerte werden gespeichert, aber im Nur-Lese-Modus nicht geregelt.':'Werte gelten für die Simulation.'}</p></div></div><label className="field">Name des Standorts<input maxLength={60} value={settings.site_name} onChange={e=>changeSettings({site_name:e.target.value})}/></label><div className="field-pair"><label className="field">Anschlussgrenze (kW)<input type="number" min="5" max="25" step="0.5" value={settings.power_limit_kw} onChange={e=>changeSettings({power_limit_kw:Number(e.target.value)})}/></label><label className="field">Grenze je Phase (A)<input type="number" min="6" max="35" value={settings.phase_limit_a} onChange={e=>changeSettings({phase_limit_a:Number(e.target.value)})}/></label></div><div className="field-pair"><label className="field">Regelreserve (kW)<input type="number" min="0.5" max="5" step="0.5" value={settings.reserve_kw} onChange={e=>changeSettings({reserve_kw:Number(e.target.value)})}/></label><label className="field">Fallback Gebäudelast (kW)<input type="number" min="0" max="20" step="0.5" value={settings.fallback_building_kw} onChange={e=>changeSettings({fallback_building_kw:Number(e.target.value)})}/></label></div><label className="field">Warteplatzrotation (s)<input type="number" min="30" max="900" step="30" value={settings.rotation_seconds} onChange={e=>changeSettings({rotation_seconds:Number(e.target.value)})}/></label><button className="button solid" disabled={disabled||!dirtySettings} onClick={async()=>{if(await command('settings',settings))setDirtySettings(false);}}><Check size={16}/> Einstellungen speichern</button></section><section className="panel form-panel architecture"><div className="panel-heading"><div><h2>So arbeitet dein System</h2><p>Die Regelung bleibt vor Ort.</p></div></div>{[{icon:Cloud,title:'Cloudflare Worker',text:'React-Oberfläche und geschützte API-Weiterleitung.'},{icon:ShieldCheck,title:'Lokale Python-Instanz',text:active?'Schreibt sichere Stromlimits direkt an die KEBA-Wallboxen.':commissioning?'Liest die echten Wallboxen im lokalen Netz aus.':'Ein eigenständiger Regler verteilt die verfügbare Leistung.'},{icon:History,title:'SQLite',text:'Einstellungen, Ereignisse und Verlauf bleiben lokal gespeichert.'},{icon:PlugZap,title:'Monta',text:'Freischaltung und Abrechnung bleiben weiterhin über OCPP bei Monta.'}].map(s=><div className="architecture-row" key={s.title}><s.icon size={22}/><div><strong>{s.title}</strong><p>{s.text}</p></div></div>)}<div className="info-note">{active?'Aktive Hardwaresteuerung: Der Fallback-Wert wird als Gebäudelast reserviert; jede Wallbox erhält einen 10-Sekunden-Failsafe.':commissioning?'Nur-Lese-Modus: Modbus-Schreibzugriffe sind gesperrt.':'Hardwaresteuerung ist im Simulationsmodus deaktiviert.'}</div></section></div>}
...
function SettingsHelp({settings}:{settings:Settings}){
  ...
  return <section className="panel settings-help" aria-labelledby="settings-help-title">
    <div className="panel-heading"><div><h2 id="settings-help-title">Was bedeuten diese Werte?</h2><p>Die Vorschau wird direkt aus deinen aktuellen Eingaben berechnet.</p></div><ShieldCheck size={20}/></div>
    <div className="settings-help-grid">
      ...
      <article><strong>Anschlussgrenze</strong><p>Maximale Gesamtleistung für Gebäude und Ladepunkte zusammen. Die Software überschreitet diesen kalkulierten Wert nicht.</p></article>
      ...
      <article><strong>Regelreserve</strong><p>Sicherheitspuffer für Schwankungen und Rundungen. Diese Leistung wird grundsätzlich nicht an Fahrzeuge verteilt.</p></article>
      <article><strong>Fallback-Gebäudelast</strong><p>Angenommener Verbrauch ohne Stromzähler. Er muss mindestens so hoch wie der höchste plausible gleichzeitige Gebäudeverbrauch sein.</p></article>
      <article><strong>Warteplatzrotation</strong><p>Ändert bei knapper Leistung nach {settings.rotation_seconds} Sekunden die Reihenfolge wartender Fahrzeuge. Priorisierte Ladepunkte bleiben zuerst an der Reihe.</p></article>
    </div>
    <div className="budget-preview">
      <div><span>Kalkuliertes Ladebudget</span><strong>{number(powerBudget)} kW</strong><small>{number(settings.power_limit_kw)} − {number(settings.fallback_building_kw)} − {number(settings.reserve_kw)} kW</small></div>
...
function EnergyFlow({state}:{state:State}){
  ...
  const values=[state.building_kw,granted,state.settings.reserve_kw];
  ...
  return <section className="control-deck" aria-label="Live-Regelbudget">
    <div className="control-deck-copy"><span className="control-kicker"><Radio size={14}/> LIVE CONTROL</span><h2>{number(granted)} kW für Fahrzeuge freigegeben</h2><p>{number(state.building_kw)} kW Fallback · {number(state.settings.reserve_kw)} kW Reserve · Grenze {number(limit,0)} kW</p></div>
    <div className="energy-stack" aria-label={`${number(used)} von ${number(limit)} Kilowatt verplant`}>
      <span className="energy-building" style={{width:`${Math.min(100,state.building_kw/limit*100)}%`}} title="Fallback-Gebäudelast"/>
    ...
    <div className="energy-legend"><span><i className="building"/>Gebäude {number(state.building_kw)} kW</span><span><i className="charging"/>Freigabe {number(granted)} kW</span><span><i className="reserve"/>Reserve {number(state.settings.reserve_kw)} kW</span><strong>{number(free)} kW frei</strong></div>
...
function StationCard({station:s,disabled,globalPaused,hardware,controllable,requested,onStart,onPause,onPriority}:{station:Station;disabled:boolean;globalPaused:boolean;hardware:boolean;controllable:boolean;requested:boolean;onStart:()=>void;onPause:()=>void;onPriority:()=>void}){
  ...
  return <article className={`station-card station-card-v2 ${s.status==='charging'?'is-charging':''}`}>
    ...
    <div className="station-kpis"><div><span>Gemessen</span><strong>{number(s.current_a)} A</strong></div><div><span>Freigabe</span><strong>{number(target,0)} A</strong></div><div><span>Gerätelimit</span><strong>{number(s.hardware_limit_a??s.max_current_a,0)} A</strong></div></div>
    <p className="station-reason">{s.connection_detail||reasons[s.status]}</p>
    {controllable&&<div className="station-actions station-actions-v2"><button disabled={disabled} className={s.priority==='high'?'priority high':'priority'} onClick={onPriority}><ArrowUpRight size={15}/>{s.priority==='high'?'Hohe Priorität':'Normal priorisiert'}</button>{s.paused||s.status==='waiting'?<button disabled={disabled||globalPaused||!s.connected} className="button solid station-command" onClick={onStart}><Play size={15}/> {requested?'Angefordert':'Jetzt laden'}</button>:<button disabled={disabled||globalPaused||!s.connected} className="button secondary station-command" onClick={onPause}><Pause size={15}/> Pausieren</button>}</div>}
