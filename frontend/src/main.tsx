  ...
  return <div className="app-shell">
    ...
    {!state?<div className="loading"><LoaderCircle className="spin"/><h2>{offline?'Auf lokale Instanz warten':'Ladepark wird verbunden'}</h2><p>Messwerte werden von der lokalen FastAPI abgerufen.</p></div>:<>
      ...
      {page==='overview'&&<>
        ...
        <section className="station-grid">{state.stations.map(s=><StationCard key={s.id} station={s} disabled={disabled} globalPaused={state.settings.paused} hardware={hardware} controllable={Boolean(active||state.mode==='simulation')} onPause={()=>void command('station',{paused:!s.paused},s.id)} onPriority={()=>void command('station',{priority:s.priority==='high'?'normal':'high'},s.id)}/>)}</section>
...
function StationCard({station:s,disabled,globalPaused,hardware,controllable,onPause,onPriority}:{station:Station;disabled:boolean;globalPaused:boolean;hardware:boolean;controllable:boolean;onPause:()=>void;onPriority:()=>void}){
  return <article className={`station-card ${s.status==='charging'?'is-charging':''}`}><div className="station-top"><div className="station-icon"><PlugZap size={23}/></div><span className={`station-status ${s.status}`}><span/>{labels[s.status]}</span></div><h3>{s.name}</h3><div className="station-model">KEBA {s.model} <span>{hardware?(s.host||'IP fehlt'):'3-phasig'}</span></div><div className="station-power">{number(s.power_kw)} <span>kW</span></div><div className="station-track"><i style={{width:`${s.current_a/s.max_current_a*100}%`}}/></div><div className="station-details"><span>{s.current_a} / {s.max_current_a} A</span><span>{hardware?`${number(s.energy_kwh||0,2)} kWh Zähler`:`${number(s.session_kwh,2)} kWh simuliert`}</span></div><p className="station-reason">{s.connection_detail||reasons[s.status]}</p>{controllable&&<div className="station-actions"><button title="Priorität ändern" aria-label={`Priorität ${s.name}`} disabled={disabled} className={s.priority==='high'?'priority high':'priority'} onClick={onPriority}><ArrowUpRight size={13}/>{s.priority==='high'?'Priorisiert':'Normal'}</button><button aria-label={`${s.paused?'Fortsetzen':'Pausieren'} ${s.name}`} title={globalPaused?'Gesamter Ladepark ist pausiert':s.paused?'Fortsetzen':'Pausieren'} disabled={disabled||globalPaused} className="icon-button" onClick={onPause}>{s.paused?<Play size={16}/>:<Pause size={16}/>}</button></div>}</article>;
}
